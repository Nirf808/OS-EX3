...
#include <vector>  //std::thread_vector
...
public:
	// gets a single pair (K1, V1) and calls emit2(K2,V2, thread_vector) any
	// number of times to output (K2, V2) pairs.
	virtual void map(const K1* key, const V1* value, void* context) const = 0;

	// gets a single K2 key and a thread_vector of all its respective V2 values
	// calls emit3(K3, V3, thread_vector) any number of times (usually once)
